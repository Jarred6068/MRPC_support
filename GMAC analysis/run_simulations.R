...
#for(i in 1:length(tissues.vec[,1])){
  ...
  #if(i == 1){
    write.table(ot1, file = "/mnt/ceph/jarredk/GMACanalysis/master_tables/simulations_adipose_subcutaneous.txt", col.names = T, row.names = F, sep = "\t", 
                quote=F)
  #}else{
  #  write.table(ot1, file = "/mnt/ceph/jarredk/GMACanalysis/master_tables/simulations_wholeblood.txt", col.names = F, row.names = F, sep = "\t",
  #              quote=F, append = T)
  #}
