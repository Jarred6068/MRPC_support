...
$\bullet$ variance across tissues plots
...
sim$alternative=(sim$het+2*sim$ha)/(2*rowSums(sim[,32:34]))
sim$reference=1-sim$alternative
genotypes=sim[,32:34]/rowSums(sim[,32:34])

#floor variables

sim$Perm.p.GMAC.floored=sim$Perm.p.GMAC
sim$Perm.p.GMAC.floored=replace(sim$Perm.p.GMAC.floored, which(sim$Perm.p.GMAC.floored==0), 2e-16)

sim$Perm.p.MRPC.floored=sim$Perm.p.MRPC
sim$Perm.p.MRPC.floored=replace(sim$Perm.p.MRPC.floored, which(sim$Perm.p.MRPC.floored==0), 2e-16)

#LTM-STM permutation pvalues 
sim$STM.perm.p.floored=sim$STM.perm.p
sim$STM.perm.p.floored=replace(sim$STM.perm.p.floored, which(sim$STM.perm.p.floored==0), 2e-16)

sim$LTM.perm.p.floored=sim$LTM.perm.p
sim$LTM.perm.p.floored=replace(sim$LTM.perm.p.floored, which(sim$LTM.perm.p.floored==0), 2e-16)

sim$TGM.perm.floored=sim$TGM.perm
sim$TGM.perm.floored=replace(sim$TGM.perm.floored, which(sim$TGM.perm.floored==0), 2e-16)
...
A1=ggplot(data = sim)+
   ...
   labs(y = expression(log10('Sim. Permutation P-value')),
        x = expression(log10('GMAC Obs. Nominal P-value')),
        subtitle = "Small True Model Simulation")+
   scale_x_continuous(limits = c(-15,0))+
...
$\bullet$ boxplots for PC inclusion differences
...
$\bullet$ variation differences between sig and non-sig trios GMAC/MRPC
