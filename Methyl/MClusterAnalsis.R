...
dfs=c(2,3,4,6)
# the -1 is for the restriction that proportions sum to 1
# 1v2 E   2P+2M+1sigma - 1 -2 = 2 
# 1v2 V   2p+2M+2sigma - 1 -2 = 3
# 1v3 E   3p+3M+1sigma - 1 -2 = 4
# 1v3 V   3p+3M+3sigma - 1 -2 = 6
...
for(i in 1:its){
  ...
  print(hist(Mdata[,i])$counts)
  
  cp1[i,] = mclustLoglik(mclustBIC(na.omit(Mdata[,i]), G=1))
  cp2[i,] = mclustLoglik(mclustBIC(na.omit(Mdata[,i]), G=2))
  cp3[i,] = mclustLoglik(mclustBIC(na.omit(Mdata[,i]), G=3))
...
row.names(P.values)=colnames(Mdata)[1:its]

cluster.cols=which(P.values[,1]<0.05)

for(i in 1:length(cluster.cols)){
  
  #pre-transformation:
  png(paste("/mnt/ceph/jarredk/Methyl/ClusCheckPlots/","Hist_", colnames(Mdata)[cluster.cols[i]], ".png", sep = ""))
  hist(Mdata[,cluster.cols[i]], 
       xlab = "Normalized Methyl Data",
       main = paste(colnames(Mdata)[cluster.cols[i]]))
