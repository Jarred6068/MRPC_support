...
where $T^{\ast}_{j}$ is the simulated trans gene, the coefficients are replaced by their estimates from the regression in \textbf{eq (1)}, the errors are standard normal, and ${\bf W}_{ij}$ is a subset of confounders in ${\bf X}_{ij}$ representing the "highly" significant confounders from \textbf{eq (1)} ($p<0.001$). Note that if the GMAC inferred mediation type was trans gene mediation only then the cis gene was simulated and the mediation test was preformed on the $\beta_1$ coefficient from regression of the cis gene: $C_i = \beta_0 + \beta_1T_j + \beta_2L_i + {\bf \Gamma X}_{ij}+\epsilon$. 
...
\textbf{Figure 4} and \textbf{Figure 5} 
...
A=ggplot(data = sim)+
 ...
 # ggtitle("Scatter of Mediation Pvalues", subtitle = "Small True Model")+
  labs(y = expression(log10('Simulated Mediation P-value')),
       x = expression(log10('Observed Mediation P-value')),
...
sim$adj.p4=sim$Perm.p
sim$adj.p4[sim$adj.p4<=2e-16]=2e-16+rnorm(length(which(sim$adj.p4<=2e-16)), 0, sd=sqrt(2e-16))

A1=ggplot(data = sim)+
  #geom_point(aes(x=Perm.p, y=STM.med.p, color="red"))+
  geom_point(aes(x=log10(adj.p4), y=log10(adj.p2), color=colvar))+
  theme(legend.position = "none")+
  labs(y = expression(log10('Simulated Mediation P-value')),
       x = expression(log10('Observed Permuted P-value')),
       subtitle = "Small True Model")+
  scale_x_continuous(limits = c(-15,0))+
  scale_y_continuous(limits = c(-15,0))+
  geom_abline(intercept = 0, slope = 1, linetype="dotted")+
...
B1=ggplot(data = sim)+
  #geom_point(aes(x=Perm.p, y=LTM.med.p, color="red"))+
  geom_point(aes(x=log10(adj.p4), y=log10(adj.p3), color=colvar))+
  theme(legend.position = "none")+
  labs(y = expression(log10('Simulated Mediation P-value')),
       x = expression(log10('Observed Permuted P-value')),
       subtitle = "Large True Model")+
...
```{r, fig.cap="$\\bf (A)$ and $\\bf (B):$ The relationship between the simulated mediation $p$-value and the number of additional (STM scenario) or missing (LTM scenario) confounders in the analysis model relative to the true model generating the trans gene. In both plots blue points represent trios with a rare allele observed at the SNP loci e.g having a frequency among subjects below $5\\%$.", echo=FALSE, message=FALSE, warning=FALSE}
#sim=read.table(file="C:/Users/Bruin/Documents/GitHub/MRPC_support/GMAC analysis/master_tables/simulations.txt", sep="\t", header = T)
...
A=ggplot(data = sim)+
  #geom_point(aes(x=Perm.p, y=STM.med.p, color="red"))+
  geom_point(aes(x=STM.dim.diff, y=log10(adj.p2), color=colvar))+
  theme(legend.position = "none")+
  #ggtitle("Scatter of Mediation Pvalues", subtitle = "Small True Model")+
  labs(y = expression(log10('Simulated Mediation P-value')),
...
B=ggplot(data = sim)+
  #geom_point(aes(x=Perm.p, y=LTM.med.p, color="red"))+
  geom_point(aes(x=LTM.dim.diff, y=log10(adj.p3), color=colvar))+
  theme(legend.position = "none")+
  #ggtitle("Scatter of Mediation Pvalues", subtitle = "Large True Model")+
  labs(y = expression(log10('Simulated Mediation P-value')),
...
A=ggplot(aes(x=Per.var.GMAC-Per.var.MRPC),data = sim.sub)+geom_histogram(aes(y = ..density..),color = "black", fill = "white")+
  ...
  labs(x = "Difference In % Variation",
       col = expression('% Difference \nIn Variation \nIn Included \nPC\'s'),
...
C=ggplot(data = sim)+
  geom_point(aes(x=Perm.p, y=Med.pvalue.GMAC, color=colvar, label=Trio.Num))+
  #geom_point(aes(x=Med.pvalue.GMAC, y=STM.med.p))+
  theme(legend.position = "none")+
  #ggtitle("Scatter of Mediation Pvalues", )+
    labs(y = "Observed GMAC Mediation Pvalue",
       x = 'Observed Permuted P-value',
       subtitle = "Comparing Mediation and Permutation Test")+
...
alternative=(sim.odd[,23]+2*sim.odd[,24])/(2*rowSums(sim.odd[,22:24]))
genotypes=sim.odd[,22:24]/rowSums(sim.odd[,22:24])
