---
title: "ADDIS Summary of Mediators GTExV8"
...
write.path.addis="C:/Users/Bruin/Documents/GitHub/MRPC_support/Reg_Net/Tables ADDIS/"
...
trans.med=df$am1t2$Trans.Gene.ID
cis.med=df$am1t1$Cis.Gene.ID
...
#write.csv(summary1, file = paste0(write.path.addis, "Summary.Mediators.csv"), row.names = FALSE)


unq.inter.med=unique(inter.med)
unq.trans.med=unique(trans.med)[-match(unq.inter.med, unique(trans.med))]
unq.cis.med=unique(cis.med)[-match(unq.inter.med, unique(cis.med))]
unq.total.med=length(unq.inter.med)+length(unq.trans.med)+length(unq.cis.med)

summary2=rbind.data.frame(cbind.data.frame(c(length(unq.cis.med), length(unq.trans.med),length(unq.inter.med)), 
                          c(length(unq.cis.med)/unq.total.med, length(unq.trans.med)/unq.total.med,
                            length(unq.inter.med)/unq.total.med)),
                          c(unq.total.med, 1))
colnames(summary2)=c("Total.Unique.Genes", "Percent.Of.Total")
summary2=cbind.data.frame(Description = c("Cis Only Unique", "Trans Only Unique","Both Cis & Trans", "Total"),
                          summary2)

library(knitr)
kable(summary2, caption = "Total number of Unqiue Cis and Trans genes identified as Mediators under MRPC-ADDIS")
write.csv(summary2, file = paste0(write.path.addis, "Summary.Mediators.unique.csv"), row.names = FALSE)
...
IX.inter1=match.rowN(df=df$am1t2$Trans.Gene.ID, target.vec = inter.med)
IX.inter2=match.rowN(df=df$am1t1$Cis.Gene.ID, target.vec = inter.med)
inter.unique.trans=df$am1t2[IX.inter1,]
inter.unique.cis=df$am1t1[IX.inter2,]
...
kable(percent.both.types[,c(1,2)], caption="ADDIS gene types for genes found as both Cis and Trans Mediators",
      col.names=c("Gene.Type", "Percent"),row.names = FALSE)

write.csv(percent.ACasM.types, file = paste0(write.path.addis, "percent.unq.ACasM.types.csv"), row.names = FALSE)
write.csv(percent.ATasM.types, file = paste0(write.path.addis, "percent.unq.ATasM.types.csv"), row.names = FALSE)
write.csv(percent.both.types[,c(1,2)], file = paste0(write.path.addis, "percent.both.types.csv"), row.names = FALSE)
...
types_am1t1=types4table(am1t1.unique$cis.Gene.type)
types_am1t2=types4table(am1t2.unique$trans.Gene.type)
...
table.am1t1=process.table(types_am1t1)
table.am1t2=process.table(types_am1t2)
table.wholegenome=process.table(types_wholegenome)

kable(table.am1t1, caption = "ADDIS Unique cis mediator gene types after grouping all gene types into 4 categories: pseudogene, protein_coding, lncRNA, and Others")

kable(table.am1t2, caption = "ADDIS Unique trans mediator gene types after grouping all gene types into 4 categories: pseudogene, protein_coding, lncRNA, and Others")
...
write.csv(table.am1t1, file = paste0(write.path.addis, "am1t1.types.4cats.csv"), row.names = FALSE)
write.csv(table.am1t2, file = paste0(write.path.addis, "am1t2.types.4cats.csv"), row.names = FALSE)
write.csv(table.wholegenome, file = paste0(write.path.addis, "whole.genome.types.4cats.csv"), row.names = FALSE)
...
The resulting $2\times 2$ contingency table is given in \textbf{table 11 and 12}. The test yielded a $\chi_{(1)}^2 = 43.77, \ p =  1.489e^{-09}$ and we therefore rejected the null hypothesis of no dependence. From \textbf{tables 11 and 12} the resulting rejection is due to an approximately $10\%$ difference in the number of pseudogenes between Cis and Trans gene positions. However, under the current test conditions, we are unable to determine if the trans position is $10\%$ enriched with pseudogenes or is the cis position is $10\%$ depleted. 
...
write.csv(cont.table.disp, file = paste0(write.path.addis, "pseudogene.comp.csv"), row.names = FALSE)
...
The test resulted in a $\chi_{(1)}^2 = 43.77, \ p = 3.68e^{-11}$ and we therefore rejected the null hypothesis of equality of observed and expected proportions. \textbf{Table 13} indicates that the rejection is due to a roughly $5.4\%$ increase of pseudogene gene type among trans mediators relative to the entire genome. 
...
mat2=rbind.data.frame(summary(cis.types), summary(all.types.adj))
per2=rbind(summary(cis.types)/sum(summary(cis.types)), summary(all.types.adj)/sum(summary(all.types.adj)))
mat2=cbind.data.frame(mat2[,1], per2[,1] ,mat2[,2], per2[,2])
colnames(mat2)=c("non-lncRNA", '%non-lncRNA',"lncRNA", '%lncRNA')
row.names(mat2)=c("Observed","Genome")

kable(mat1, caption = "Chi-Square GOF observed vs. Expected proportions of trans gene  Type=pseudogene", digits=4)
kable(mat2, caption = "Chi-Square GOF observed vs. Expected proportions of cis gene: Type=pseudogene", digits=4)
write.csv(mat1, file = paste0(write.path.addis, "pseudo.comp.wg.trans.csv"), row.names = TRUE)
write.csv(mat2, file = paste0(write.path.addis, "pseudo.comp.wg.cis.csv"), row.names = TRUE)
...
Similar to the above tests, we also explored the dependence among gene position and gene type with types consisting of protein coding or non-protein coding. Again, we conducted a Chi-square test of independence (\textbf{tables 14 and 15}). The test yielded a $\chi_{(1)}^2 = 189.98, \ p \approx 0$ and we rejected the null hypothesis of no dependence. The rejection is due to an approximately $1:2$ ratio of protein coding to non-protein coding gene types among the cis mediators and an approximately $1:5$ ratio of protein coding to non-protein coding gene types among the trans mediators. 
...
cont.table2=cbind.data.frame(cont.table2[,1], round(cont.table2[,1]/sum(cont.table2[,1]),4), cont.table2[,2], round(cont.table2[,2]/sum(cont.table2[,2]),4))
...
write.csv(cont.table.disp2, file = paste0(write.path.addis, "protein.coding.comp.csv"), row.names = FALSE)
...
colnames(mat2)=c("non-lncRNA", '%non-lncRNA',"lncRNA", '%lncRNA')
row.names(mat2)=c("Observed","Genome")
kable(mat1, caption = "Chi-Square GOF observed vs. Expected proportions of trans gene  Type=protein_coding", digits=4)
kable(mat2, caption = "Chi-Square GOF observed vs. Expected proportions of cis gene: Type=protein_coding", digits=4)
write.csv(mat1, file = paste0(write.path.addis, "prc.comp.wg.trans.csv"), row.names = TRUE)
write.csv(mat2, file = paste0(write.path.addis, "prc.comp.wg.cis.csv"), row.names = TRUE)
...
write.csv(cont.table.disp3, file = paste0(write.path.addis, "lncRNA.comp.csv"), row.names = FALSE)
...
kable(mat1, caption = "Chi-Square GOF observed vs. Expected proportions of trans gene  Type=lncRNA", digits=4)
kable(mat2, caption = "Chi-Square GOF observed vs. Expected proportions of cis gene: Type=lncRNA", digits=4)
write.csv(mat1, file = paste0(write.path.addis, "lncRNA.comp.wg.trans.csv"), row.names = TRUE)
write.csv(mat2, file = paste0(write.path.addis, "lncRNA.comp.wg.cis.csv"), row.names = TRUE)
```
