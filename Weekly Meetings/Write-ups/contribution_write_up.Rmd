...
\indent $\rightarrow$ All associated stored data files in: $/mnt/ceph/jarredk/AddisReRunFiles$      
$\rightarrow$ Rerun script located in $/mnt/ceph/jarredk/ADDIS\_verify/ADDIS\_ReRun.R$    

$\bullet$ Summarizing of differences between \textbf{LOND} and \textbf{ADDIS}    
$\rightarrow$ quantified the distribution of trio classifications for each tissue as counts of (M0, M1, ... M4, Other) - table can be found in $/mnt/ceph/jarredk/ADDIS\_verify/Trios\_analysis\_GTEx\_v8\_allPCs\_V3\_ordered\_updated.xlsx$    
$\rightarrow$ 
...
$\bullet$ Just as the p-value gives the expected False Positive Rate (FPR) by rejecting any hypotheses with a p-value at or below the FPR, the q-value similarly gives the Positive False Discovery Rate (pFDR)/type I error by rejecting any hypothesis with a q-value at or below the pFDR
