...
- Example_run.R: to run a small example with just ??? SNPs, which finishes in ~1 min
- Run_script.R: runs on all the SNPs for all the chromosomes
- calc....R: contains the functions called by the above two R scripts
...
It then runs the correlation calculation function, which takes a sec.
It generates two output text files: xxx1 and xxx2.
...
A short example script EXrun.R is included as well as example .txt files for the genotype data and associated genotype meta data. 
...
- Format of the example genotype data file (filename):

- Format of the example genotype metadata file (filename):
