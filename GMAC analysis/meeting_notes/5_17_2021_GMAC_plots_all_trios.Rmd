...
\indent $\bullet$ \textbf{Purpose:} We simulated 10,000 trios of all model types (M0 - M4) with $u$ "unknown" confounding variables (PCs) represented by the matrix ${\bf U}$ and 3 simulated "known" confounders represented by the matrix ${\bf K}$.
...
\noindent \textbf{(2.)} - We denote the $j^{th}$ molecular pheonotype as $T_j$ and its set of parents by ${\bf Parents}(T_j)$. The set of parent nodes ${\bf Parents}(T_j)$ may be empty, contain $V$-nodes, $T$-nodes, or a combination of both (Badsha and Fu, 2019). We model $T_j$ as following a normal distribution where
