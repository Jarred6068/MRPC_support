...
filepath = "/mnt/ceph/jarredk/Methyl/Correlation_Calculation/CCv3/"
...
# The filenames are automatically generated by function calc.corsV3 below 

# load functions in calcCOR_v2.R for correlation calculation
source(file = paste(filepath, "calcCOR_v3.R", sep = ""))
...
genotype_data=read.table(file = paste(input.location, "ExampleY_Genotype_data.txt", sep = ""), header=T)
# genotype metadata: a meta data matrix of 1000 SNPs in the rows and 3 columns [SNP_ID    chr  coordinate]
G_metadata=read.table(file = paste(input.location, "ExampleY_genotype_Metadata.txt", sep = ""), header = T)
