...
####################
# genotype data: a data matrix with 1000 SNPs in the rows and 1000 subjects in the columns
genotype_data=read.table(file = paste(input.location, "Example_Genotype_data_small.txt", sep = ""), sep="\t", header=T)
# genotype metadata: a meta data matrix of 1000 SNPs in the rows and 3 columns [SNP_ID    chr  coordinate]
G_metadata=read.table(file = paste(input.location, "Example_Genotype_Metadata_small.txt", sep = ""), sep = "\t", header = T)
# Methylation metadata: a meta data matrix of 470,158 probes and 3 columns [Ilmn_ID    chr    coordinate]
...
# Methylation Data: a matrix of 606 individuals in rows and 470,158 probes in columns
